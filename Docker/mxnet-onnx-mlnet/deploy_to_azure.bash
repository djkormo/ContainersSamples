...
az acr build --registry $ACR_NAME --image ai-onnx:v1 .
...
# szczegoly 
az acr repository show -n $ACR_NAME -t ai-onnx:v1
...
az container create \
    ...
    --image $ACR_LOGIN_SERVER/ai-onnx:v1 \
    --registry-login-server $ACR_LOGIN_SERVER \
    --registry-username $(az keyvault secret show --vault-name $AKV_NAME -n $ACR_NAME-pull-usr --query value -o tsv) \
    --registry-password $(az keyvault secret show --vault-name $AKV_NAME -n $ACR_NAME-pull-pwd --query value -o tsv) \
    --dns-name-label ai-onnx$RND \
	--ports 80 \
    --ip-address=Public \
...
clear
# koty
curl -X POST http://ai-onnx$RND.northeurope.azurecontainer.io/image -F imageData=@images/cat1.jpg
curl -X POST http://ai-onnx$RND.northeurope.azurecontainer.io/image -F imageData=@images/cat2.jpg

# psy
curl -X POST http://ai-onnx$RND.northeurope.azurecontainer.io/image -F imageData=@images/dog1.jpg
curl -X POST http://ai-onnx$RND.northeurope.azurecontainer.io/image -F imageData=@images/dog2.jpg

# konie
curl -X POST http://ai-onnx$RND.northeurope.azurecontainer.io/image -F imageData=@images/horse1.jpg
curl -X POST http://ai-onnx$RND.northeurope.azurecontainer.io/image -F imageData=@images/horse2.jpg
